...
const data: { label: string; x: number; y: number }[] = [
	{ label: "0", x: randomInt(), y: randomInt() },
	{ label: "1", x: randomInt(), y: randomInt() },
];
...
const graph = new LineChart(canvas, width, height, data, {
	...
	onHover: {
		func: (info) => {
			...
			graphInfoElem.innerText = `${data.label}: {x: ${data.x}, y: ${data.y}}`;
